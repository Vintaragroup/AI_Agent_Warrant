...
async def preview_page(req: Request, case_id: str):
    ...
    og_img   = f"{settings.BASE_URL}/static/og/{case_id}.jpg"
...
async def api_checkin(request: Request, token: str = Form(...), photo: UploadFile | None = None,
    ...
    try:
        ...
        to = case.get("phone")
        if to:
            # enqueue a follow-up in your real scheduler; we just log in this starter
...
async def admin_last_area(req: Request, case_id: str):
    last = logs.find_one({"case_id": case_id, "geo": {"$ne": None}}, sort=[("ts", -1)]) or            checkins.find_one({"case_id": case_id, "geo": {"$ne": None}}, sort=[("ts", -1)])
...
async def admin_send_link(case_id: str):
    ...
    await log_event(case_id, Request(scope={"type":"http","headers":[],"client":("0.0.0.0",0)}),
                    evt_type="sms_sent", person_id=case.get("person_id"),
                    notes="admin_send_link", extra={"to": case["phone"]})
    return {"ok": True}
